 ...
 *形成8位一个字节的形式。 如果剩下的字符不足3个字节，则用0填充，输出字符使用'='，因此编码后输出的文本末尾可能
 *会出现1或2个'='。
 *　　为了保证所输出的编码位可读字符，Base64制定了一个编码表，以便进行统一转换。编码表的大小为2^6=64，这也是Base64
 * 名称的由来。
 * @author hyp
 * @version V1.0  
 * @Title: Base64Util.java
 * @Package com.jarvis.base.util
 * @Description:Base64工具类,文件和字节的Base64加密解密工具
...
public final class Base64Util {
    ...
    private static final int TWENTYFOURBITGROUP = 24; //四个bit一组

    private static final int EIGHTBIT = 8; //8bit

    private static final int SIXTEENBIT = 16; //16bit
    ...
    private static final byte PAD = (byte) '='; //”=“转为byte
    /**
     * 文件读取缓冲区大小
     */
